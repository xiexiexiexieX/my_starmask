"""
Mask R-CNN (mmdet baseline) — ResNet-50 FPN

用法:
  python mmdet/mask_rcnn/train.py                 # 全量训练
  python mmdet/mask_rcnn/train.py --epochs 1      # 冒烟测试
  python mmdet/mask_rcnn/train.py --resume        # 续训

输出: work_dirs/mmdet/mask_rcnn/
"""
...
# RTX 4080 example: --batch-size 8 --num-workers 4 --fast
...
CONFIG = os.path.join(ROOT, "mmdet/configs_real_mixed/mask_rcnn_star.py")
...
class Tune:
    ...
    patience   = 10
    use_amp    = True
    work_dir   = os.path.join(ROOT, "work_dirs/real_mixed_baselines/mmdet/mask_rcnn")
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Mask R-CNN (mmdet baseline)")
    ...
    ap.add_argument('--work-dir', type=str, default=os.path.join(DEFAULT_WORK_ROOT, "mask_rcnn"))
