...
This module mirrors the project-root ``npy_loading.py`` for compatibility with
local imports. MMDetection configs should still prefer ``imports=['npy_loading']``
on Windows to avoid colliding with the installed OpenMMLab ``mmdet`` package.
"""
import os
import json
