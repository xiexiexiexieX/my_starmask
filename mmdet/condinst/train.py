"""
CondInst (mmdet baseline) — 条件卷积实例分割

用法:
  python mmdet/condinst/train.py                 # 全量训练
  python mmdet/condinst/train.py --epochs 1      # 冒烟测试

输出: work_dirs/mmdet/condinst/
"""
...
CONFIG = os.path.join(ROOT, "mmdet/configs_real_mixed/condinst_star.py")
...
class Tune:
    ...
    patience   = 10
    use_amp    = True
    work_dir   = os.path.join(ROOT, "work_dirs/real_mixed_baselines/mmdet/condinst")
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="CondInst (mmdet baseline)")
    ...
    ap.add_argument('--work-dir', type=str, default=os.path.join(DEFAULT_WORK_ROOT, "condinst"))
