...
class AnchorGenerator:
    def __init__(self, strides=(2, 4, 8, 16), base_sizes=(4, 8, 16, 32),
                 scales=(1.0, 1.26, 1.587), ratios=(0.25, 0.5, 1.0, 2.0, 4.0)):
...
class RPN(nn.Module):
    def __init__(self, anchor_gen, in_ch=256, pos_iou=0.7, neg_iou=0.3,
        ...
        self.max_loss_anchors = max_loss_anchors
    ...
    def _loss(self, cls_flat, reg_flat, anchors, gt_boxes_list):
        ...
        for b in range(B):
            ...
            anchors_b = anchors
            if anchors_b.shape[0] > self.max_loss_anchors:
                if gt.numel() > 0:
                    with torch.no_grad():
                        centers = (anchors_b[:, 0:2] + anchors_b[:, 2:4]) * 0.5
                        gt_centers = (gt[:, 0:2] + gt[:, 2:4]) * 0.5
                        dist = torch.cdist(centers, gt_centers).min(dim=1).values
                        near_k = min(self.max_loss_anchors // 2, anchors_b.shape[0])
                        near_idx = dist.topk(near_k, largest=False).indices
                        rand_k = self.max_loss_anchors - near_idx.numel()
                        rand_idx = torch.randperm(anchors_b.shape[0], device=anchors_b.device)[:rand_k]
                        keep_idx = torch.unique(torch.cat([near_idx, rand_idx]))[:self.max_loss_anchors]
                else:
                    keep_idx = torch.randperm(anchors_b.shape[0], device=anchors_b.device)[:self.max_loss_anchors]
                anchors_b = anchors_b[keep_idx]
                logits = logits[keep_idx]
                deltas = deltas[keep_idx]
            if gt.numel() == 0:
                labels = torch.zeros_like(logits)
                cls_loss_total = cls_loss_total + F.binary_cross_entropy_with_logits(logits, labels, reduction='mean')
                valid_imgs += 1; continue
            with torch.no_grad():
                ious = box_iou(anchors_b, gt)
            ...
            if pos_idx.numel() > 0:
                with torch.no_grad():
                    reg_targets = encode_boxes(gt[argmax[pos_idx]], anchors_b[pos_idx])
...
class MaskHead(nn.Module):
    def __init__(self, in_ch=256, num_classes=1):
        ...
        self.coarse = nn.Conv2d(256, num_classes, 1)
        self.refine = nn.Sequential(
            nn.Conv2d(256 + num_classes, 128, 3, padding=1),
    ...
    def forward(self, x):
        ...
        residual = self.refine(torch.cat([x, coarse], dim=1))
...
class ROIHead(nn.Module):
    ...
    def _forward_train(self, feats, proposals, img_size, gt_boxes_list, gt_masks_list, gt_weights_list):
        ...
        if all_mask_rois:
            ...
            dice = dice_loss_with_logits(mask_pred, mask_t)
            w = mask_w.clamp(min=0)
            losses['roi_mask'] = ((bce + dice) * w).sum() / (w.sum() + 1e-6)
        return losses
