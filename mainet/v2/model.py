...
class MAINetRCNN(nn.Module):
    def __init__(self, in_chans=1, num_classes=1, fpn_out=256,
        ...
        super().__init__()
        self.backbone_type = 'dual_path_v2'
    ...
    def forward(self, imgs, masks_list=None, weights_list=None,
        ...
        if self.training:
            ...
            if gt_params_list is not None and params is not None and 'phi' in params:
                pd = param_supervision_loss(params, gt_params_list)
                losses['param_angle'] = pd['angle'] * param_weight_scale
                losses['param_gate']  = pd['gate']
                losses['param_phi']   = imgs.new_tensor(pd['phi'])
            else:
...
class RCNNCriterion(nn.Module):
    ...
    def forward(self, imgs, masks, wts, gt_params_list=None, param_weight_scale=1.0):
        raw = self.model(imgs, masks, wts, gt_params_list=gt_params_list,
