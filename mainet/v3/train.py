"""
MAINet v3 — DualPath v3（MoffatPSF + MultiOrientStrip + SKFusion，零标注）
==========================================================================
...
用法:
  python mainet/v3/train.py --debug --epochs 1              # 冒烟测试
  python mainet/v3/train.py --epochs 100                    # 全量训练
"""
...
class Config:
    ...
    output_dir = os.path.join(_PROJECT_ROOT, "work_dirs/mainet/v3")
...
def build_model(cfg, device):
    ...
    model = MAINetRCNN(in_chans=cfg.in_chans, num_classes=cfg.num_classes,
                       score_thr=0.3, nms_thr=0.5, max_per_img=200).to(device)
    criterion = RCNNCriterion(model)
    n = sum(p.numel() for p in model.parameters())
    print(f"Model (RCNN/v3 MoffatPSF+MultiOrientStrip+SKFusion): {n/1e6:.2f}M params")
...
def train_one_epoch(model, criterion, loader, optimizer, device,
    ...
    for imgs, masks, wts, params, _ in pbar:
        ...
        pbar.set_postfix({
            ...
            'cls':f"{meters['class']/n_seen:.3f}", 'reg':f"{meters.get('rpn_reg',0)/n_seen:.3f}",
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(description="MAINet v3 — MoffatPSF + MultiOrientStrip + SKFusion")
